 ...
 */
public class BodyWriter implements AutoCloseable {
    ...
    private final AtomicBoolean sizeExceeded = new AtomicBoolean(false);
    ...
    public void write(final ByteBuffer buffer) {
        if (!isClosed.get()) {
            if (sizeExceeded.get() || stream.size() >= maxSize) {
                sizeExceeded.compareAndSet(false, true);
                return;
            }
            try {
